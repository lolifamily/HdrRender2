...
// coverage. The engine draws the UI straight onto the opaque FinalLDR and nobody reads that alpha. We clear FinalLDR
// into a transparent UI layer first (UiLayerPatch) and composite it premultiplied over the HDR scene, so vector elements
// need their coverage in alpha: new_a = src_a + dst_a * (1 - src_a), i.e. One / InverseSourceAlpha. RGB is unchanged:
// SrcAlpha / InvSrcAlpha over a transparent background is color * alpha, premultiplied, exactly what the composite expects.
//
...
// builds VectorRenderer's PSOs with it, and UiLayerVectorPatch uses them only while the UI draws into the UI layer.
//
